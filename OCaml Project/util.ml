...
module UtilM = struct
  ...
  let all_parts (lst: 'a list): ('a list * 'a list) list =
    let rev_lst = List.rev lst in
    let rec pre_order (op_lst: 'a list) (remain: 'a list): ('a list * 'a list) list =
      match op_lst with
     | [] -> if lst=[] then [([],[])] else [([],lst)]
     | x::xs -> (List.rev op_lst,remain)::(pre_order xs (List.rev(List.rev(remain) @ [x])))
      in
      pre_order rev_lst []
